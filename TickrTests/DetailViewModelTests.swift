...
final class DetailViewModelTests: XCTestCase {
    ...
    func testYahooProviderFailureYieldsFailedState() async throws {
        let provider = ThrowingCandleProvider(error: YahooCandleProviderError.httpError(statusCode: 503))
        let model = DetailViewModel(
            symbol: "AAPL",
            quoteProvider: MockQuoteProvider(),
            candleProvider: provider,
